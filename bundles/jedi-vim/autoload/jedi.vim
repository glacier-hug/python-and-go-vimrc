...
" Jedi motion functions


fun! jedi#move(pattern, flags, ...) "{{{
    let cnt = v:count1 - 1
...
fun! jedi#vmove(pattern, flags) range "{{{
    call cursor(a:lastline, 0)
    let end = jedi#move(a:pattern, a:flags)
...
fun! jedi#select(pattern, inner) "{{{
    ...
    while cnt
        ...
        let cnt = cnt - 1
    endwhile
    if jedi#pos_le([snum, 0], orig) && jedi#pos_le(orig, [enum, 1])
        if a:inner
            let snum = snum + 1
